...
const About = () => {
    ...
    return (
        <div className={`${darkMode ? 'bg-slate-600 border-white shadow-slate-400' : 'bg-black border-black shadow-black'} w-1/2 m-5 p-4 min-w-lg  shadow-2xl  flex flex-col gap-4 justify-center items-center`}>
            ...
            <p className={`text-lg text-white`}>{darkMode ? 'Why do programmers prefer dark mode?' : ''} { darkMode && <span className={`opacity-0 hover:opacity-100`}> Because light attracts bugs!&#128514;</span>}</p>
        </div>
    )
}
